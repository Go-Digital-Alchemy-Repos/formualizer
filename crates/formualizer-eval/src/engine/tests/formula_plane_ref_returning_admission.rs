...
fn assert_cells_equal(
    ...
    // Errors are compared by kind. This fork keeps rich error metadata
    // (message/context/extra, GOD-338) beside the Arrow error code on the
    // legacy authority, but the experimental FormulaPlane publishes through
    // `span_eval::literal_to_overlay`, which keeps only `map_error_code(kind)`.
    // Upstream's engine has no rich metadata at all, so kind parity is exactly
    // what this test measured upstream; carrying rich errors through the plane
    // is separate work, and `FormulaPlaneMode::Off` is the shipped default.
    fn comparable(value: Option<LiteralValue>) -> Option<LiteralValue> {
        match value {
            Some(LiteralValue::Error(error)) => {
                Some(LiteralValue::Error(ExcelError::new(error.kind)))
            }
            other => other,
        }
    }
    ...
    for (row, col) in cells {
        assert_eq!(
            comparable(left.get_cell_value(sheet, row, col)),
            comparable(right.get_cell_value(sheet, row, col)),
            "{sheet}!R{row}C{col}"
        );
    }
}
...
fn reference_returning_error_empty_and_short_circuit_semantics_match_legacy() {
    ...
    for (col, kind) in [
        ...
        // `=IF($D$1:$D$3>0,$B$2,$C$2)`. Upstream answers #VALUE! because IF
        // never lifts element-wise. Under CL-087/ES-008 this fork lifts IF's
        // condition slot, so the call spills 3x1 and lands on #SPILL! here,
        // where the two rows below hold the same fill-down formula. Both
        // authorities agree on it (the parity sweep above covers that); only
        // the expected kind moves.
        (15, ExcelErrorKind::Spill),
