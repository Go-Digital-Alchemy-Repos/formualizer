...
/// `parts` with an XLDAPR `xl/metadata.xml` record, and cells authored the way
/// Excel saves a dynamic array (`<f t="array" ref="A1">` plus `cm="1"`).
///
/// Upstream treats every formula as dynamic, so its spill fixtures wrote a bare
/// `<f>SEQUENCE(2)</f>`. This fork classifies a bare `<f>` as a legacy scalar
/// (OT-035 / CL-016 authored-array semantics) and implicitly intersects it, so
/// that cell answers 1 and never attempts a spill. Declaring the cell dynamic
/// keeps these tests about what they measure -- how the recalculate package
/// handles a spill-class result -- rather than about loader classification.
fn dynamic_parts(rows: &str) -> BTreeMap<String, String> {
    ...
    parts.insert(
        ...
        parts["xl/_rels/workbook.xml.rels"].replace(
            "</Relationships>",
            &format!(
                "<Relationship Id=\"rIdMetadata\" Type=\"{OFFICE}/sheetMetadata\" Target=\"metadata.xml\"/></Relationships>"
            ),
        ),
    );
    parts.insert(
        "xl/metadata.xml".to_owned(),
        format!(
            "<metadata xmlns=\"{MAIN}\" xmlns:xda=\"http://schemas.microsoft.com/office/spreadsheetml/2017/dynamicarray\"><metadataTypes count=\"1\"><metadataType name=\"XLDAPR\" minSupportedVersion=\"120000\" cellMeta=\"1\"/></metadataTypes><futureMetadata name=\"XLDAPR\" count=\"1\"><bk><extLst><ext uri=\"{{bdbb8cdc-fa1e-496e-a857-3c3f30c029c3}}\"><xda:dynamicArrayProperties fDynamic=\"1\" fCollapsed=\"0\"/></ext></extLst></bk></futureMetadata><cellMetadata count=\"1\"><bk><rc t=\"1\" v=\"0\"/></bk></cellMetadata></metadata>"
        ),
    );
    parts
}
...
fn unsupported_spill_does_not_return_a_partial_package() {
    // A spill is only reachable from a cell authored as a real dynamic array
    // (see `dynamic_parts`), and the recalculator refuses such a package up
    // front. What this test measures -- no partial package is ever returned for
    // a workbook the recalculator cannot fully compute -- is unchanged.
    let p = dynamic_parts(
...
fn modern_scalar_errors_are_cached_and_can_be_recalculated_again() {
    // Upstream also drove `SEQUENCE(2)` -> `#SPILL!` here. That case is not
    // reachable in this fork through a package the recalculator accepts: a bare
    // `<f>` is a legacy scalar under OT-035 / CL-016 authored-array semantics
    // and is implicitly intersected (so it answers 1, never `#SPILL!`), while a
    // cell authored as a real dynamic array carries XLDAPR cell metadata, which
    // the recalculator refuses up front as unsupported. The spill-class refusal
    // itself is covered by `unsupported_spill_does_not_return_a_partial_package`.
    for (formula, token) in [("FILTER(A2:A2,FALSE)", "#CALC!")] {
        let p = parts(&format!(
