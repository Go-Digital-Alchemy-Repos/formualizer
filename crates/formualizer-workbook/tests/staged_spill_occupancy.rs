...
//
// A1 is authored the way Excel saves a dynamic array: `<f t="array" ref="A1">`
// plus `cm="1"` selecting an XLDAPR record in `xl/metadata.xml`. Upstream treats
// every formula as dynamic, so upstream's fixture wrote a bare `<f>`; this fork
// classifies a bare `<f>` as a legacy scalar (OT-035 / CL-016 authored-array
// semantics) and implicitly intersects it, so a bare `<f>SEQUENCE(2)</f>` would
// answer 1 and never attempt a spill at all. Declaring the cell dynamic keeps
// what this test is about -- pending-source occupancy blocking a spill -- and
// takes the loader classification out of it.
fn fixture(blocker: &str) -> Vec<u8> {
    ...
    let parts = [
        ("[Content_Types].xml", "<Types xmlns=\"http://schemas.openxmlformats.org/package/2006/content-types\"><Default Extension=\"rels\" ContentType=\"application/vnd.openxmlformats-package.relationships+xml\"/><Default Extension=\"xml\" ContentType=\"application/xml\"/><Override PartName=\"/xl/workbook.xml\" ContentType=\"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet.main+xml\"/><Override PartName=\"/xl/worksheets/sheet1.xml\" ContentType=\"application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml\"/><Override PartName=\"/xl/metadata.xml\" ContentType=\"application/vnd.openxmlformats-officedocument.spreadsheetml.sheetMetadata+xml\"/></Types>".into()),
        ...
        ("xl/_rels/workbook.xml.rels", format!("<Relationships xmlns=\"{rels}\"><Relationship Id=\"rId1\" Type=\"{office}/worksheet\" Target=\"worksheets/sheet1.xml\"/><Relationship Id=\"rIdMetadata\" Type=\"{office}/sheetMetadata\" Target=\"metadata.xml\"/></Relationships>")),
        ("xl/worksheets/sheet1.xml", format!("<worksheet xmlns=\"{main}\"><sheetData><row r=\"1\"><c r=\"A1\" cm=\"1\"><f t=\"array\" ref=\"A1\">SEQUENCE(2)</f></c><c r=\"D1\"><f>7</f></c></row><row r=\"2\"><c r=\"A2\"><f>{blocker}</f></c></row></sheetData></worksheet>")),
        ("xl/metadata.xml", format!("<metadata xmlns=\"{main}\" xmlns:xda=\"http://schemas.microsoft.com/office/spreadsheetml/2017/dynamicarray\"><metadataTypes count=\"1\"><metadataType name=\"XLDAPR\" minSupportedVersion=\"120000\" cellMeta=\"1\"/></metadataTypes><futureMetadata name=\"XLDAPR\" count=\"1\"><bk><extLst><ext uri=\"{{bdbb8cdc-fa1e-496e-a857-3c3f30c029c3}}\"><xda:dynamicArrayProperties fDynamic=\"1\" fCollapsed=\"0\"/></ext></extLst></bk></futureMetadata><cellMetadata count=\"1\"><bk><rc t=\"1\" v=\"0\"/></bk></cellMetadata></metadata>")),
    ];
