...
pub struct CalamineAdapter {
    ...
    cancel: Option<CancelToken>,
    /// OOXML worksheet part paths by sheet name, resolved on first request.
    worksheet_paths: OnceLock<BTreeMap<String, String>>,
...
impl CalamineAdapter {
    ...
    /// OOXML worksheet part paths by sheet name, resolved on first request.
    ///
    /// The fork's array-formula and saved-value scans address worksheet parts
    /// directly, so they need the workbook-relation mapping that Calamine does
    /// not expose. Resolved lazily like the other source scans.
    fn lazy_worksheet_paths(&self) -> &BTreeMap<String, String> {
        self.worksheet_paths
            .get_or_init(|| Self::scan_worksheet_paths_from_reader(self.cancellable_reader()))
    }
    ...
    fn scan_dynamic_array_cell_metadata(
        ...
        Self::scan_dynamic_array_cell_metadata_from_reader(self.cancellable_reader(), limits)
    }
    ...
    fn scan_array_formula_metadata(
        ...
        let worksheet_path = self.lazy_worksheet_paths().get(sheet).ok_or_else(|| {
            Self::array_formula_metadata_error(format!(
